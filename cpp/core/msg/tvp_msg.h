...
TVP_MSG_DEFINE(TVPPluginUnboundFunctionError,
...
TVP_MSG_DEFINE(TVPInfoLoadingConfigurationFileFailed,
...
TVP_MSG_DEFINE(TVPBasicDrawDeviceTextureHasAlreadyBeenLocked,
               "BasicDrawDevice: Texture has already been locked (StartBitmapCompletion() has been "
               "called twice without EndBitmapCompletion()),) unlocking the texture.")
...
TVP_MSG_DEFINE(TVPInfoNotFoundScreenModeFromDriver,
               "(info) Panic! There is no reasonable candidate screen mode provided from the "
               "driver ... trying to use the default screen size and color depth ...")
...
TVP_MSG_DEFINE(TVPAllScreenModeError,
...
TVP_MSG_DEFINE(TVPD3dErrDriverIinternalError,
               "D3D : Internal driver error. Applications should destroy and recreate the device "
               "when receiving this error. ")
TVP_MSG_DEFINE(TVPD3dErrInvalidCall,
               "D3D : The method call is invalid. For example, a method's parameter may not be a "
               "valid pointer.")
...
TVP_MSG_DEFINE(TVPD3dErrUnsuportedAalphtOperation,
               "D3D : The device does not support a specified texture-blending operation for the "
               "alpha channel.")
TVP_MSG_DEFINE(TVPD3dErrUnsuportedAlphaArg,
               "D3D : The device does not support a specified texture-blending argument for the "
               "alpha channel.")
TVP_MSG_DEFINE(TVPD3dErrTooManyOperations,
               "D3D : The application is requesting more texture-filtering operations than the "
               "device supports.")
...
TVP_MSG_DEFINE(TVPD3dErrUnsuportedFactorValue,
               "D3D : The device does not support the specified texture factor value. Not used; "
               "provided only to support older drivers.")
TVP_MSG_DEFINE(TVPD3dErrConflictioningRenderState,
               "D3D : The currently set render states cannot be used together.")
TVP_MSG_DEFINE(TVPD3dErrUnsupportedTextureFilter,
...
TVP_MSG_DEFINE(TVPD3dErrWasStillDrawing,
               "D3D : The previous blit operation that is transferring information to or from this "
...
TVP_MSG_DEFINE(TVPD3dOkNoAutoGen,
               "D3D : This is a success code. However,) the autogeneration of mipmaps is not "
...
TVP_MSG_DEFINE(TVPExceptionAccessViolation,
               "Access Violation: The thread attempts to read from or write to a virtual address "
...
TVP_MSG_DEFINE(
    ...
    "Data Type Misalignment: The thread attempts to read or write data that is misaligned on "
    "hardware that does not provide alignment. For example, 16-bit values must be aligned on "
    "2-byte boundaries,) 32-bit values on 4-byte boundaries,) and so on.")
TVP_MSG_DEFINE(TVPExceptionSingleStep,
               "Single Step: A trace trap or other single instruction mechanism signals that one "
               "instruction is executed.")
TVP_MSG_DEFINE(TVPExceptionArrayBoundsExceeded,
               "Array Bounds Exceede: The thread attempts to access an array element that is out "
               "of bounds, and the underlying hardware supports bounds checking.")
TVP_MSG_DEFINE(
    TVPExceptionFltDenormalOperand,
    "Denormal Operand: One of the operands in a floating point operation is denormal. A denormal "
    "value is one that is too small to represent as a standard floating point value.")
TVP_MSG_DEFINE(TVPExceptionFltDivideByZero,
               "Divide By Zero: The thread attempts to divide a floating point value by a floating "
               "point divisor of 0 (zero).")
TVP_MSG_DEFINE(TVPExceptionFltInexactResult,
               "Inexact Result: The result of a floating point operation cannot be represented "
...
TVP_MSG_DEFINE(TVPExceptionFltOverflow,
               "Overflow: The exponent of a floating point operation is greater than the magnitude "
               "allowed by the corresponding type.")
TVP_MSG_DEFINE(
    TVPExceptionFltStackCheck,
...
TVP_MSG_DEFINE(TVPExceptionFltUnderflow,
               "Underflow: The exponent of a floating point operation is less than the magnitude "
               "allowed by the corresponding type.")
TVP_MSG_DEFINE(TVPExceptionIntDivideByZero,
               "Divide By Zero: The thread attempts to divide an integer value by an integer "
               "divisor of 0 (zero).")
TVP_MSG_DEFINE(
    TVPExceptionIntOverflow,
    "Overflow: The result of an integer operation creates a value that is too large to be held by "
    "the destination register. In some cases, this will result in a carry out of the most "
    "significant bit of the result. Some operations do not set the carry flag.")
TVP_MSG_DEFINE(TVPExceptionPrivInstruction,
               "Priv Instruction: The thread attempts to execute an instruction with an operation "
               "that is not allowed in the current computer mode.")
TVP_MSG_DEFINE(TVPExceptionNoncontinuableException,
               "Noncontinuable Exception: The thread attempts to continue execution after a "
               "non-continuable exception occurs.")
...
TVP_MSG_DEFINE(TVPExceptionInPageError,
               "In Page Error: The thread tries to access a page that is not present, and the "
               "system is unable to load the page. For example, this exception might occur if a "
               "network connection is lost while running a program over a network.")
TVP_MSG_DEFINE(TVPExceptionInvalidDisposition,
               "Invalid Disposition: An exception handler returns an invalid disposition to the "
               "exception dispatcher. Programmers using a high-level language such as C should "
               "never encounter this exception.")
TVP_MSG_DEFINE(TVPExceptionInvalidHandle,
               "Invalid Handle: The thread used a handle to a kernel object that was invalid "
